...
/**
 * Inject a pre-configured database for integration tests.
 * Only call this before any route handler runs (i.e. before the first request).
 * Never use in production code — the function name is intentionally ugly.
 */
export function _setDbForTesting(testDb: Database.Database): void {
  db = testDb;
}

// ── Analysis file freshness ────────────────────────────────────────────────────
// These files are generated by scripts/ab-test-presets.ts, scripts/pr-roc.ts,
// and scripts/rolling-error.ts.  We regenerate them sequentially in the
// background on every server start so they're never silently stale.

const ANALYSIS_SCRIPTS = [
  { key: "ab-test",       script: "scripts/ab-test-presets.ts", out: "data/ab-test.json" },
  { key: "pr-roc",        script: "scripts/pr-roc.ts",          out: "data/pr-roc.json" },
  { key: "rolling-error", script: "scripts/rolling-error.ts",   out: "data/rolling-error.json" },
] as const;

function readAnalysisMeta(): Record<string, { generatedAt: string | null; ageMinutes: number | null }> {
  const now = Date.now();
  const result: Record<string, { generatedAt: string | null; ageMinutes: number | null }> = {};
  for (const { key, out } of ANALYSIS_SCRIPTS) {
    try {
      const raw = JSON.parse(readFileSync(path.join(__dirname, out), "utf8")) as { generatedAt?: string };
      const generatedAt = raw.generatedAt ?? null;
      const ageMinutes = generatedAt
        ? Math.round((now - new Date(generatedAt).getTime()) / 60000)
        : null;
      result[key] = { generatedAt, ageMinutes };
    } catch {
      result[key] = { generatedAt: null, ageMinutes: null };
    }
  }
  return result;
}

function regenerateAnalysisFiles(): void {
  const tsxBin = path.join(__dirname, "node_modules", ".bin", "tsx");
  let i = 0;
  const runNext = (): void => {
    if (i >= ANALYSIS_SCRIPTS.length) return;
    const { key, script } = ANALYSIS_SCRIPTS[i++];
    logger.info({ script: key }, "regenerating analysis file");
    ...
    proc.on("exit", (code) => {
      if (code === 0) {
        logger.info({ script: key }, "analysis file regenerated");
      } else {
        logger.warn({ script: key, exitCode: code }, "analysis file regeneration failed");
      }
      runNext();
    });
  };
